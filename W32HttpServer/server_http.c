...
int read_file_to_buffer(const char* file_path, char* file_buffer, u32 buffer_size, u32* bytes_read) {
    ...
    if (NULL == file_path || NULL == file_buffer || NULL == bytes_read || buffer_size == 0) {
        return -1;
    }
    
    *bytes_read = 0;
    
    file_handle = CreateFile(file_path, GENERIC_READ, FILE_SHARE_READ, NULL, 
                           OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    
    if (file_handle == INVALID_HANDLE_VALUE) {
        return -1; /* File not found or access denied */
    }
    ...
    if (file_size > buffer_size - 1) {
        file_size = buffer_size - 1; /* Truncate if too large */
    }
    
    result = ReadFile(file_handle, file_buffer, file_size, &actual_bytes_read, NULL);
    CloseHandle(file_handle);
    ...
    file_buffer[actual_bytes_read] = '\0'; /* Null terminate for safety */
...
int build_http_file_response(const char* file_path, const char* mime_type, const char* file_data, u32 file_size, char* response_buffer, u32 buffer_size, u32* response_length) {
    ...
    if (NULL == mime_type || NULL == file_data || NULL == response_buffer || NULL == response_length || buffer_size == 0) {
        return -1;
    }
    
    /* Build HTTP header */
    header_len = wsprintf(response_buffer,
        "HTTP/1.0 200 OK\r\n"
        "Content-Type: %s\r\n"
        "Content-Length: %u\r\n"
        "Connection: close\r\n"
    ...
    if (header_len < 0 || (u32)header_len >= buffer_size) {
        return -1; /* Header too large */
    }
    
    /* Check if we have room for the file data */
    if ((u32)header_len + file_size >= buffer_size) {
        return -2; /* Response too large for buffer */
    }
    
    /* Copy file data after header */
    memcpy(response_buffer + header_len, file_data, file_size);
    *response_length = (u32)header_len + file_size;
